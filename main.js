...
(function () {
  ...
  /* ═══════════════════════════════════════════
     3D BANNER CARD INTERACTIVE TILT EFFECT
     ═══════════════════════════════════════════ */
  var bannerCard = document.getElementById('bannerCard');
  if (bannerCard) {
    var shine = bannerCard.querySelector('.banner-shine');
    var imgContainer = bannerCard.querySelector('.banner-image-container');

    bannerCard.addEventListener('mousemove', function (e) {
      var rect = bannerCard.getBoundingClientRect();
      ...
      // Tilt angles (max 22 degrees)
      var rotateY = px * 22;
      var rotateX = -py * 22;

      // Apply 3D rotation and scale
      bannerCard.style.transform = 'perspective(1200px) rotateX(' + rotateX + 'deg) rotateY(' + rotateY + 'deg) scale3d(1.04, 1.04, 1.04)';

      // Parallax shifts inside the container (move image slightly opposite)
      if (imgContainer) {
        imgContainer.style.transform = 'translate3d(' + (px * -10) + 'px, ' + (py * -10) + 'px, -20px) scale(1.22)';
      }

      // Shine effect gradient position
      if (shine) {
        var shineX = (x / rect.width) * 100;
        var shineY = (y / rect.height) * 100;
        shine.style.background = 'radial-gradient(circle at ' + shineX + '% ' + shineY + '%, rgba(255, 215, 0, 0.22) 0%, rgba(255, 255, 255, 0) 65%)';
    ...
    bannerCard.addEventListener('mouseleave', function () {
      ...
      bannerCard.style.transform = 'perspective(1200px) rotateX(0deg) rotateY(0deg) scale3d(1, 1, 1)';
      if (imgContainer) {
        imgContainer.style.transform = 'translate3d(0px, 0px, -20px) scale(1.22)';
      }
      if (shine) {
